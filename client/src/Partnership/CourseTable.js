...
const CourseTable = () => {
  ...
  const generatePDFReport = () => {
    ...
    // Get page height and place the logo at the bottom
    const pageHeight = doc.internal.pageSize.height;
    const logoHeight = 20; // Adjust based on your image's actual height
    doc.addImage(img, 'PNG', 160, pageHeight - logoHeight - 10, 40, logoHeight); // Logo is now 10 units from the bottom
  
    // Add title at the top
    doc.text("Course Details Report", 20, 10);
  
    // Generate table with course details
    doc.autoTable({
      startY: 20, // Adjust starting point to avoid overlapping with title
